...
function buildDocument(app) {
  ...
  // 获取tags&paths--判断是否是数组
  let dirScanners = Array.isArray(swagger.dirScanner) ? swagger.dirScanner : [swagger.dirScanner];
  for (let dirScanner of dirScanners) {
    let filepath = path.join(app.config.baseDir, dirScanner);
    // 递归获取 tags&paths
    let subTag_Path = getTag_Path(filepath, securitys, swagger, definitions);
    // 合并子目录的扫描结果
    tag_path.tags = tag_path.tags.concat(subTag_Path.tags);
    Object.assign(tag_path.paths, subTag_Path.paths);
  }
